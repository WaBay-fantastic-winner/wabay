...
export default class extends Controller {
  ...
  connect() {
    ...
    document.querySelector('.total').value = this.element.dataset.price
  }

  subtract() {
    this.amountTarget.value <= 1 ? this.amountTarget.value = 1 : this.amountTarget.value -- ;
    this.sumTarget.textContent = this.amountTarget.value * this.element.dataset.price + Number(this.extraDonateTarget.value)
    document.querySelector('.total').value = this.amountTarget.value * this.element.dataset.price + Number(this.extraDonateTarget.value)
  }
